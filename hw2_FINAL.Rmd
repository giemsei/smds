...
1. Derive the ML estimator of $\pi$.

**Solution**

Using that we have independent observation, we can write the joint distribution as follow:
$$f(y_1, \dots, y_n)= \Pi_{i=1}^n (1-\theta)^{y_i-1}\theta $$
Now let's express the liklihood function and manipulate it:
\begin{align*}
\ell(\theta) &= \Pi_{i=1}^n (1-\theta)^{y_i-1}\theta \\
&= (1-\theta)^{(y_1-1)+ \dots (y_n - 1)}\theta^n \\
&=  (1-\theta)^{\sum_{i=1}^{n}(y_i - 1)}\theta^n \\
&= \theta^n  (1-\theta)^{n\bar{Y}-n} \\
&= \theta^n  (1-\theta)^{n\bar{Y}-n} \times 1
\end{align*}
Since only the statistic $\bar{Y} =\frac{1}{n} \sum_{i=1}^{n}y_i$ shows up in the part of $\ell(\theta)$ involving $\theta$, which is the parameter, by factorization theorem we have that $\bar{Y}$ is a sufficient statistic. 

Now we use $\ell(\theta)$ to find the ML estimator for $\theta$.
\begin{align*}
0 = \dfrac{d\ell(\theta)}{d\theta} &=  (n-\sum_{i=1}^{n}y_i) (1-\theta)^{\sum_{i=1}^{n}(y_i - 1)-1}\theta^n + n(1-\theta)^{\sum_{i=1}^{n}(y_i - 1)}\theta^{n-1}\\
...
prop.test(success,n,h0,alt="two.sided",conf.level=0.95,correct=FALSE)

```

The p-value for this test is equal to 0.166. This P-value is not very small, so the evidence against the $H_0$ is not strong. It is plausible that $\pi=0.50$ and so is not appropriate to accept $H_0$. There isn't sufficient evidence to determine whether a majority or minority of the population prefer to raise taxes or to reduce services!
...
Pooled standard deviation and $se$:
...
```
P-value is between 0.01 and 0.05, so we have evidence against the null hypothesis.
Moreover, p-level is less than the level $\alpha = 0.05$, so we would reject.
